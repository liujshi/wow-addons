...
sArena:SetSize(60, 25)
sArena:SetBackdrop(BackdropLayout)
sArena:SetBackdropColor(0, 0, 0, 0)
...
sArena.Title:SetText(AddonName .. "")
...
function sArena:Test(numOpps)
	...
	local _, _, _, specIcon = GetSpecializationInfo(64)
...
function sArena:ARENA_PREP_OPPONENT_SPECIALIZATIONS(self)
	local numOpps = GetNumArenaOpponentSpecs();
	for i=1, MAX_ARENA_ENEMIES do
		if (i <= numOpps) then
			local ArenaFrame = _G["ArenaEnemyFrame"..i];
			local ArenaFrameSpecBorder=_G["ArenaEnemyFrame"..i.."SpecBorder"]
			local ArenaFrameName=_G["ArenaEnemyFrame"..i.."Name"]
			local ArenaFrameTexture=_G["ArenaEnemyFrame"..i.."Texture"]
			-- specID= GetArenaOpponentSpec(i);
			-- if (specID > 0) then 
			-- local _, _, _, specIcon, _, _, class = GetSpecializationInfoByID(specID);
			-- print(specIcon)
			-- SetPortraitToTexture(ArenaFrame.classPortrait, specIcon);
			-- end
		ArenaFrameSpecBorder:SetTexture()
		--ArenaFrame.classPortrait:SetTexture()
		ArenaFrame.specPortrait:Hide()
		ArenaFrameName:Hide()
		ArenaFrameTexture:Hide()
		--ArenaFrame:Show()
		end
	end	
end

sArena:RegisterEvent("ARENA_PREP_OPPONENT_SPECIALIZATIONS")
local HealthBars = {
